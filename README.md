...
| `2aa166a` | **A failed UDP listener is retired, not abandoned**: `udpPacketLoop` used to `break` on any read error, leaving the socket bound but unread — under the `3927e02` SO_REUSEPORT sharding the kernel keeps hashing 1/N of all datagrams into it, so it black-holes that share once the buffer fills, silently (the `net.ErrClosed` branch logs at Debug). Seen live: 3 of 4 listeners dead after ~17h, 37% of UDP datagrams dropped at the socket, `dig` answering 4/12, while DoH/DoT/DoQ stayed healthy. `udpLoopActionFor` decides continue/retire/stop; retiring **closes** the socket (which is what removes it from the REUSEPORT group) and reopens a replacement, increments `udpListenersRetired` and logs at Error |
...
| 2026-09-04 | `v0.84.2` | **One patch taken:** `e66fa7f`, adapting `096ff91` (AGH-32) — the RFC 9250 §4.5 0-RTT opcode check, reachable here because DoQ is served with `Allow0RTT`.  The release's only other commit (`9b6551d`) is a **Go directive bump**, 1.26.6 → 1.26.8 — one line of `go.mod` plus five CI/Docker strings — which this fork does not need, as builds run on Go 1.27.1.  **Correction (2026-09-04, same day):** this row first called that commit a Go *and* quic-go v0.59.0 → v0.60.0 bump, and `e66fa7f`'s commit message says the same. Wrong, and the diffstat already quoted here showed it — `go.mod | 2 +-` is one line changed. Upstream pinned quic-go v0.60.0 from **`v0.82.0`** onward — this row first said `v0.84.0`, also wrong: `v0.81.1`–`v0.81.4` carry v0.59.0, every tag from `v0.82.0` carries v0.60.0 — so the v0.59.0 pinned here is a **standing delta two minor releases old**, unrelated to this release. The conclusion is unchanged: `ConnectionState().Used0RTT` exists in v0.59.0, so the port needs no bump. |
| 2026-09-24 | `v0.85.0` | **Nothing taken — reviewed, no port needed.**  Five commits; four are test and docs refactors (AGH-148, AGH-149, AGH-151) plus a move to golibs `validate.Interface`, with no `go.mod` change.  The one fix, `ddf250f` (ADG-12585), replaces `conn.Read` with `io.ReadFull` for the 2-byte TCP/DoT length prefix: a short read decoded the length as 0 and desynced the stream.  This fork has read the prefix with `io.ReadFull` since `bbd79ad` (2026-05-23); a split-prefix test over `net.Pipe` fails on upstream `v0.84.2` (`got 0 bytes, want 29`) and passes here.  `requiresValidation` dropping its `*net.UDPAddr` cast was not taken: the QUIC transport here sits on `net.ListenUDP`, so the cast cannot fail.  Upstream #525 (UDP listener abandoned on read error) is still open and unfixed in this release; `2aa166a` remains the fork's answer. |
